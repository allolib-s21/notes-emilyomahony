...
#include <vector>  // for tracking looped notes
...
class MyApp : public App {
 public:
  ...
  void playNote(float frequency, float time) {
    ...
    voice->setTriggerParams({0.2, frequency, 0.1,0.1,0.0});
    synthManager.synthSequencer().addVoiceFromNow(voice, time, 1.5);
  }

  void loop_note(float frequency) {
    for (int i = 1; i <= 16; i++) {
      playNote(frequency, i * 16);
    }
  }
  ...
  bool onKeyDown(Keyboard const& k) override {
    ...
    if (k.shift()) {
      ...
      if (midiNote > 0) {
        ...
        // loop stuff
        if (recording) {
          // loop.push_back(Note(pow(2.f, (midiNote - 69.f) / 12.f) * 432.f, )))
