...
public:
  ...
  virtual void onInit( ) override {
    ...
    void loop_note(float frequency) {
        for (int i = 1; i <= 16; i++) {
            ...
            synthManager.synthSequencer().addVoiceFromNow(voice, i * 8, note_len);
        }
    }
    ...
    bool onKeyDown(Keyboard const& k) override {
        ...
        if (k.shift()) {
            ...
            if (midiNote > 0) {
                if (k.alt() && midiNote > 44) {
                    midiNote -= 24;
                }
                Sub *v = synthManager.synth().getVoice<Sub>();
                synthManager.configureVoiceFromGui(v);
                v->setInternalParameterValue(
                    "frequency", ::pow(2.f, (midiNote - 69.f) / 12.f) * 432.f);
                // synthManager.triggerOn(midiNote);
                synthManager.synthSequencer().addVoiceFromNow(v, 0, note_len);
                if (recording) {
                    loop_note(::pow(2.f, (midiNote - 69.f) / 12.f) * 432.f);
                }
            }
        }
        return true;
